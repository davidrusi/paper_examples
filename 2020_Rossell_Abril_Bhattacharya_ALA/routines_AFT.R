...
##
## - ROUTINES TO SIMULATE DATA FROM SCENARIO 1: TRUTH IS X1 beta1 + log(abs(X2)) beta2
##
## - ROUTINES TO RUN VARIABLE SELECTION ON SIMULATED DATA
##
...
#Cox LASSO
fitCoxLASSOCV <- function(y, x, maxit=10000) {
  require(glmnet)
  if (class(y) != 'Surv') stop("y must be of class 'survival'")
  resp= cbind(time=exp(y[,1]),status=y[,2])
  cv.fit= try(cv.glmnet(x, y=resp, family="cox", maxit=maxit, nfolds=10, alpha=1), silent=TRUE)
  fit= try(glmnet(x, y=resp, family = "cox", maxit=maxit, alpha=1), silent=TRUE)
...
#####################################################################################################
# ROUTINES TO SIMULATE DATA FROM SCENARIO 1 OF ROSSELL & RUBIO 2019
#####################################################################################################
...
simScenario= function(seed,beta,corr,sigmae,n=100,censtimes=1,errors='normal',asym=0,priorCoef= momprior(tau=0.192), priorDelta= modelbbprior(1,1), priorGroup=groupzellnerprior(taustd=1), method, nknots=9) {
  ...
  if (missing(method)) stop("You must specify method=='ALA' or method=='LA'")
  p= length(beta)
  sim= simDataScen1(seed=seed,beta=beta,n=n,corr=corr,sigmae=sigmae,censtimes=censtimes,errors=errors,asym=asym)
  y= sim$y; X= sim$X
  mslin= modelSelection(y ~ X[,1] + X[,2], priorCoef=priorCoef, priorDelta=priorDelta, priorGroup=priorGroup, verbose=FALSE, method=method)
  ms= modelSelection(y ~ X[,1] + X[,2], smooth=~ X[,1] + X[,2], priorCoef=priorCoef, priorDelta=priorDelta, priorGroup=priorGroup, nknots=nknots, verbose=FALSE, method=method)
  pplin= postProb(mslin)
  pp= postProb(ms)
  #Post-process output
  idx1= which(ms$groups==1 | ms$groups==3)
  idx2= which(ms$groups==2 | ms$groups==4)
  pplin= pplinWithoutIntercept(pplin)
  S1= which(ms$groups==3)
  S2= which(ms$groups==4)
  ppsplines= data.frame(ppWithoutIntercept(pp, L1=2, L2=3, S1=S1, S2=S2))
  ans= list(pplin=pplin,ppsplines=ppsplines)
  cat(".")
  return(ans)
}
...
simScenarioWithSpur= function(seed,beta,corr,sigmae,n=100,censtimes=1,errors='normal',asym=0,smoothterms=TRUE,nknots=9,verbose=FALSE,priorCoef= momprior(tau=0.192), priorDelta= modelbbprior(1,1), priorGroup=groupzellnerprior(tau=n),usevars=1:length(beta),method='Laplace') {
  ...
  # - method: 'ALA' for approximate Laplace approximation, 'Laplace' for Laplace approximation
  #
  # OUTPUT
  # - margpp: marginal posterior inclusion probabilities 
  # - topmodel: highest posterior probability model (censored data)
  if (missing(method)) stop("You must specify method=='ALA' or method=='Laplace'")
  p= length(beta)
  sim= simDataScen1(seed=seed,beta=beta,n=n,corr=corr,sigmae=sigmae,censtimes=censtimes,errors=errors,asym=asym)
  y= sim$y; X= sim$X
  #Run modelSelection
  ...
  ms= modelSelection(f, smooth=s, priorCoef=priorCoef, priorDelta=priorDelta, priorGroup=priorGroup, verbose=verbose, nknots=nknots, method=method)
  pp= pp2margpp(ms)
  ...
  names(margpp)= sub("\\.s.",".spline",names(margpp))
  #Highest posterior probability model (censored data)
  topmodel= pp$pp[1,]
  varnames= rownames(pp$margpp)[as.numeric(strsplit(as.character(topmodel$modelid),split=',')[[1]])]
  sel.smooth= sapply(1:length(beta),function(i) grep(paste("X\\[, ",i,"\\].s",sep=''),varnames)[1])
  sel.smooth= sel.smooth[!is.na(sel.smooth)]
  varnames= varnames[c(grep("\\]$",varnames), sel.smooth)]
  varnames= sub("\\.s.",".spline",varnames)
  topmodel= data.frame(varnames=paste(varnames, collapse=', '), pp=topmodel$pp)
  ans= list(margpp=margpp, topmodel=topmodel)
  cat(".")
  return(ans)
}


# Log-normal baseline hazard, parH = c(mu,sigma)
simDataScen35= function(seed, parH, betaH, beta, n, corr, censtimes) {
  TC = 50
  p <- length(beta); mu0 <- parH[1]; sigma0 <- parH[2]; 
  ...
  X0 <- X
  X0[,2] <- log(abs(X0[,2]))
  exp.xbetaH    <- exp(X0%*%betaH)
  exp.dif <- exp(X0%*%(betaH-beta))
  # Simulation
  temp.ut1  <- runif(n)
  lifestimes =  qlnorm(  1 - exp( log(temp.ut1)*exp.dif )  , meanlog = mu0, sdlog = sigma0)/exp.xbetaH  
  lifestimes =  ifelse(is.infinite(lifestimes),TC,lifestimes)
  y= Surv(time=log(pmin(lifestimes, censtimes)), event= censtimes>lifestimes)
  yuncens= y; yuncens[,1]= log(lifestimes); yuncens[,2]=1
  return(list(y=y,yuncens=yuncens,X=X))
}
...
simScenarioWithSpur35= function(seed,parH, betaH, beta,corr,n=100,censtimes=1,smoothterms=TRUE,nknots=9,verbose=FALSE,priorCoef= momprior(tau=0.192), priorDelta= modelbbprior(1,1), priorGroup=groupzellnerprior(tau=n), usevars=1:length(beta), method) {
  #Simulate data with many covariates and run modelSelection on all of them
  # - seed: seed for random number generation
  # - betaH: true GH model parameters (time scale)
  # - beta: true GH model parameters (hazard scale)
  # - corr: each row in the covariate matrix arises from a Normal with mean 0, variance 1 and pairwise correlations = corr
  # - parH: parameters of the baseline hazard
  # - n: sample size
  # - censtimes: administrative censoring time (i.e. any observation > censtimes is censored)
  ...
  # OUTPUT
  # - margpp: marginal posterior inclusion probabilities 
  # - topmodel: highest posterior probability model 
  if (missing(method)) stop("You must specify method=='ALA' or method=='Laplace'")
  p= length(beta)
  sim= simDataScen35(seed=seed,parH=parH,betaH=betaH,beta=beta,n=n,corr=corr,censtimes=censtimes)
  y= sim$y; X= sim$X
  #Run modelSelection
  f= formula(paste("y ~",paste(paste('X[,',usevars,']',sep=''),collapse='+')))
  s= formula(paste("~",paste(paste('X[,',usevars,']',sep=''),collapse='+')))
...
margpp= function(simout) {
    ...
    ans= lapply(1:length(simout), function(z) matrix(NA,nrow=2,ncol=2))
    for (i in 1:length(simout)) {
        z= simout[[i]]
        ans[[i]][1,]= c(sum(z$pplin[grep('X1',z$pplin[,1]),-1]), sum(z$pplin[grep('X2',z$pplin[,1]),-1]))
        ans[[i]][2,]= c(sum(z$ppsplines[grep('1',z$ppsplines[,1]),-1]), sum(z$ppsplines[grep('2',z$ppsplines[,1]),-1]))
        rownames(ans[[i]])= c('Linear','Linear and non-linear')
        colnames(ans[[i]])= c('X1','X2')
    }
...
averagepp= function(simout) {
    ...
    for (i in 2:length(simout)) {
        notna= !any(is.na(simout[[i]]$ppsplines[,2]))
    ...
    colnames(pplin)= colnames(ppsplines)= c('model','pp')
    ans= list(pplin=pplin,ppsplines=ppsplines)
    return(ans)
}
...
correctselmombf= function(simout,p,activevars,selcens=FALSE) {
    ...
    # - selcens: set to TRUE if simulations results stored in element 'topmodel.cens'
    ...
    if (selcens) {
        ...
        topmodel= sapply(simout, function(z) as.character(z$topmodel[1,1]))
...
#pMOM + group Zellner
dpmomgzell <- function(par, X, groups= 1:ncol(X), tau, taugroup, logscale=FALSE)  {
  colX= length(par); ans=0
  if (ncol(X)>0) {
      ningroup= table(groups)
      isgroup= groups %in% (as.numeric(names(ningroup)[ningroup>1]))
      ans= sum(dmom(par[!isgroup], tau=tau, logscale=TRUE))
      if (any(isgroup)) {
...
#Gradient and hessian of log pMOM + log group Zellner
dpmomgzellgradhess <- function(par, X, groups= 1:ncol(X), tau, taugroup)  {
  colX= length(par)
  grad= double(length(par)); hess= matrix(0,nrow=length(par),ncol=length(par))
  if (length(par)>0) {
      ningroup= table(groups)
...
dgzellgzell <- function(par, X, groups= 1:ncol(X), tau, taugroup, logscale=FALSE)  {
  colX= length(par); ans=0
  if (ncol(X)>0) {
      ans= 0
...
dgzellgzellgradhess <- function(par, X, groups= 1:ncol(X), tau, taugroup)  {
  colX= length(par)
  grad= double(length(par)); hess= matrix(0,nrow=length(par),ncol=length(par))
  if (length(par)>0) {
      ningroup= table(groups)
      ...
      for (j in groupids) {
          Sj= (t(X[,groups==j,drop=FALSE]) %*% X[,groups==j,drop=FALSE])
          if (sum(groups==j)==1) {
            grad[groups==j]= -(1/tau) * (Sj %*% matrix(par[groups==j],ncol=1))
            hess[groups==j,groups==j]= -(1/tau) * Sj
          } else {
            sel= c(groups==j,FALSE)
            grad[sel]= -(1/taugroup) * (Sj %*% matrix(par[groups==j],ncol=1))
